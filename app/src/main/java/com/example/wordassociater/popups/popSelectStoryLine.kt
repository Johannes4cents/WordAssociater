...
fun popSelectStoryLine(from: View, onStoryLineSelected: (storyLine: StoryLine) -> Unit, liveList: MutableLiveData<List<StoryLine>?>) {
    val b = PopupSelectStorylineBinding.inflate(LayoutInflater.from(from.context), null, false)
    val pop = Helper.getPopUp(b.root, from, 600, 800)

    b.storyLineRecycler.initRecycler(liveList, onStoryLineSelected, null)
}