...
class EventHolderPreview(val b: HolderSnippetPartPreviewBinding): RecyclerView.ViewHolder(b.root), LiveHolder {
    override lateinit var item: LiveClass
    override fun onBind(item: LiveClass, takeItemFunc:((item: LiveClass) -> Unit)?) {
        b.itemName.text = (item as Event).name
        Glide.with(b.root.context).load((item as Event).imgUrl).into(b.itemImage)
        setObserver()
